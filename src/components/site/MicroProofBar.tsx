  ...
  return (
    ...
    >
      ...
      <div className="container relative py-10 sm:py-14">
        <dl className="grid grid-cols-2 md:grid-cols-5 gap-y-10 md:gap-y-0 md:gap-0">
          {metrics.map((m, i) => (
            <Reveal
              key={m.label}
              delay={i * 0.06}
              className={`flex flex-col items-center text-center md:px-4 ${i > 0 ? "md:border-l md:border-white/15" : ""}`}
            >
              <dt className="sr-only">{m.label}</dt>
              <dd className="flex flex-col items-center w-full">
                <div className="font-display font-bold text-white leading-none flex items-end justify-center min-h-[44px] sm:min-h-[52px]"
                ...
                <div className="mt-3 font-mono-ui text-[11px] sm:text-[12px] uppercase tracking-[0.12em] text-white/80 px-2 min-h-[2.8em] flex items-center justify-center text-center">
                  {m.label}
                </div>
                <div className="mt-1 text-[11px] text-white/55 px-2 text-center min-h-[1.6em]">{m.sub}</div>
              </dd>
            </Reveal>
          ))}
        </dl>
