...
import { RevenueLeakChart } from "./Illustrations";
...
export default function Problem() {
  return (
    ...
    >
      <div aria-hidden className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] sm:w-[600px] h-[500px] sm:h-[600px] rounded-full pointer-events-none"
      ...
      <div className="container relative grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div>
          ...
          <h2
            id="problem-heading"
            className="text-white font-extrabold mt-5 leading-[1.1]"
            style={{ fontSize: "clamp(28px, 6vw, 3rem)", letterSpacing: "-0.04em" }}
          >
          ...
          <p className="mt-6 sm:mt-7 text-white/80 max-w-[440px] leading-[1.75]">
          ...
          <RevenueLeakChart className="mt-8 w-full max-w-[320px] h-auto" />

          <a href="#why" className="mt-7 inline-flex items-center gap-2 text-brand-bright font-bold group min-h-[44px]">
            See how Vigorant fixes this
            <ArrowRight aria-hidden size={18} className="transition-transform group-hover:translate-x-1" />
          </a>
        </div>

        <ul className="space-y-4 list-none p-0" aria-label="Key revenue-loss statistics">
...
function Stat({ n, suffix = "", prefix = "", decimals = 0, label, note }: any) {
  return (
    <motion.li
      ...
      className="rounded-2xl p-6 sm:p-7 transition-all duration-300 hover:translate-x-2 list-none"
      style={{ background: "hsl(var(--charcoal-2))", border: "1px solid hsl(0 0% 100% / 0.08)" }}
    >
      <div className="font-extrabold text-[2.2rem] sm:text-[2.8rem] leading-none gradient-text" style={{ letterSpacing: "-0.06em" }}>
        <Counter to={n} prefix={prefix} suffix={suffix} decimals={decimals} />
      </div>
      <div className="mt-3 text-white font-medium">{label}</div>
      <div className="mt-1 text-white/65 text-sm font-mono-ui">{note}</div>
    </motion.li>
  );
}
